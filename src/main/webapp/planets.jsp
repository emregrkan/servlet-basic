...
<% if (request.getAttribute("response") != null) {
    Response<PlanetDto> resp = (Response<PlanetDto>) request.getAttribute("response"); %>
            ...
            <div class="relative rounded-xl overflow-auto">
                <div class="shadow-sm overflow-hidden my-8">
                    <table class="border-collapse table-auto w-full text-sm">
                        ...
                        <% for (PlanetDto planet : resp.getResults()) { %>
                        <tr class="dark:hover:bg-slate-700 hover:bg-slate-100 handle">
                            <td class="border-b border-slate-100 dark:border-slate-700 p-4 pl-8 text-slate-500 dark:text-slate-400">
                                <a href="<%=planet.getUrl()%>"><%= planet.getName() %>
                                </a>
                            </td>
                            <td class="border-b border-slate-100 dark:border-slate-700 p-4 pl-8 text-slate-500 dark:text-slate-400"><%= planet.getSurfaceWater() %>
                            </td>
                            <td class="border-b border-slate-100 dark:border-slate-700 p-4 pl-8 text-slate-500 dark:text-slate-400"><%= planet.getPopulation() %>
                            </td>
                        </tr>
                        <% } %>
                        </tbody>
                            ...
                            <th class="font-medium p-4 pl-8 pb-0 pt-3 dark:text-slate-200 text-left">
                                <div>
                                    <a id="table-prev" class="pr-2"
                                       href="${pageContext.request.contextPath}/plantes?page=<%=((int) request.getAttribute("page")) - 1%>">Previous</a>
                                    <a id="table-next"
                                       href="${pageContext.request.contextPath}/plantes?page=<%=((int) request.getAttribute("page")) + 1%>">Next</a>
                                </div>
                            </th>
                        </tr>
                        </tfoot>
